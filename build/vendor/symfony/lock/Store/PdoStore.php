 ...
 */
class PdoStore implements \LockmeDep\Symfony\Component\Lock\PersistingStoreInterface
{
    ...
    public function __construct($connOrDsn, array $options = [], float $gcProbability = 0.01, int $initialTtl = 300)
    {
        if ($gcProbability < 0 || $gcProbability > 1) {
            throw new \LockmeDep\Symfony\Component\Lock\Exception\InvalidArgumentException(\sprintf('"%s" requires gcProbability between 0 and 1, "%f" given.', __METHOD__, $gcProbability));
        }
        if ($initialTtl < 1) {
            throw new \LockmeDep\Symfony\Component\Lock\Exception\InvalidTtlException(\sprintf('"%s()" expects a strictly positive TTL, "%d" given.', __METHOD__, $initialTtl));
        }
     ...
     */
    private function getHashedKey(\LockmeDep\Symfony\Component\Lock\Key $key) : string
    {
    ...
    public function createTable() : void
    {
        ...
        if ($conn instanceof \LockmeDep\Doctrine\DBAL\Connection) {
     ...
     */
    public function configureSchema(\LockmeDep\Doctrine\DBAL\Schema\Schema $schema) : void
    {
        if (!$this->getConnection() instanceof \LockmeDep\Doctrine\DBAL\Connection) {
    ...
    private function getDriver() : string
    {
        ...
        if ($con instanceof \PDO) {
            ...
            switch (\true) {
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\Mysqli\Driver:
                    throw new \LogicException(\sprintf('The adapter "%s" does not support the mysqli driver, use pdo_mysql instead.', static::class));
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\AbstractMySQLDriver:
                    $this->driver = 'mysql';
                    break;
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDOSqlite\Driver:
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDO\SQLite\Driver:
                    $this->driver = 'sqlite';
                    break;
                ...
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDO\PgSQL\Driver:
                    $this->driver = 'pgsql';
                    break;
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\OCI8\Driver:
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDOOracle\Driver:
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDO\OCI\Driver:
                    $this->driver = 'oci';
                    break;
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\SQLSrv\Driver:
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDOSqlsrv\Driver:
                case $driver instanceof \LockmeDep\Doctrine\DBAL\Driver\PDO\SQLSrv\Driver:
                    $this->driver = 'sqlsrv';
                    break;
                default:
    ...
    private function addTableToSchema(\LockmeDep\Doctrine\DBAL\Schema\Schema $schema) : void
    {
