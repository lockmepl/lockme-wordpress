    ...
    public function __invoke(\LockmeDep\Psr\Http\Message\RequestInterface $request, array $options) : \LockmeDep\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (\is_string($uri) && ($type = \LockmeDep\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\LockmeDep\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
    private function addExpectHeader(\LockmeDep\Psr\Http\Message\RequestInterface $request, array $options, array &$modify) : void
    {
