...
use LockmeDep\GuzzleHttp\Exception\ConnectException;
use LockmeDep\GuzzleHttp\Exception\RequestException;
use LockmeDep\GuzzleHttp\Promise as P;
use LockmeDep\GuzzleHttp\Promise\FulfilledPromise;
use LockmeDep\GuzzleHttp\Promise\PromiseInterface;
use LockmeDep\GuzzleHttp\Psr7;
use LockmeDep\GuzzleHttp\TransferStats;
use LockmeDep\GuzzleHttp\Utils;
...
class StreamHandler
{
    ...
    public function __invoke(\LockmeDep\Psr\Http\Message\RequestInterface $request, array $options) : \LockmeDep\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $startTime = isset($options['on_stats']) ? \LockmeDep\GuzzleHttp\Utils::currentTime() : null;
        try {
            ...
            if (\false !== \strpos($message, 'getaddrinfo') || \false !== \strpos($message, 'Connection refused') || \false !== \strpos($message, "couldn't connect to host") || \false !== \strpos($message, "connection attempt failed")) {
                $e = new \LockmeDep\GuzzleHttp\Exception\ConnectException($e->getMessage(), $request, $e);
            } else {
                $e = \LockmeDep\GuzzleHttp\Exception\RequestException::wrapException($request, $e);
            }
            $this->invokeStats($options, $request, $startTime, null, $e);
            return \LockmeDep\GuzzleHttp\Promise\Create::rejectionFor($e);
        }
    }
    private function invokeStats(array $options, \LockmeDep\Psr\Http\Message\RequestInterface $request, ?float $startTime, \LockmeDep\Psr\Http\Message\ResponseInterface $response = null, \Throwable $error = null) : void
    {
        if (isset($options['on_stats'])) {
            $stats = new \LockmeDep\GuzzleHttp\TransferStats($request, $response, \LockmeDep\GuzzleHttp\Utils::currentTime() - $startTime, $error, []);
     ...
     */
    private function createResponse(\LockmeDep\Psr\Http\Message\RequestInterface $request, array $options, $stream, ?float $startTime) : \LockmeDep\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        return new \LockmeDep\GuzzleHttp\Promise\FulfilledPromise($response);
    }
    private function createSink(\LockmeDep\Psr\Http\Message\StreamInterface $stream, array $options) : \LockmeDep\Psr\Http\Message\StreamInterface
    {
        ...
        if (!empty($options['decode_content'])) {
            ...
            if (isset($normalizedKeys['content-encoding'])) {
                ...
                if ($encoding[0] === 'gzip' || $encoding[0] === 'deflate') {
                    $stream = new \LockmeDep\GuzzleHttp\Psr7\InflateStream(\LockmeDep\GuzzleHttp\Psr7\Utils::streamFor($stream));
     ...
     */
    private function drain(\LockmeDep\Psr\Http\Message\StreamInterface $source, \LockmeDep\Psr\Http\Message\StreamInterface $sink, string $contentLength) : \LockmeDep\Psr\Http\Message\StreamInterface
    {
        // If a content-length header is provided, then stop reading once
        // that number of bytes has been read. This can prevent infinitely
        // reading from a stream when dealing with servers that do not honor
        // Connection: Close headers.
        \LockmeDep\GuzzleHttp\Psr7\Utils::copyToStream($source, $sink, \strlen($contentLength) > 0 && (int) $contentLength > 0 ? (int) $contentLength : -1);
        $sink->seek(0);
        $source->close();
        return $sink;
    }
     ...
     * @return resource
     */
    private function createStream(\LockmeDep\Psr\Http\Message\RequestInterface $request, array $options)
    {
        static $methods;
        if (!$methods) {
    ...
    private function resolveHost(\LockmeDep\Psr\Http\Message\RequestInterface $request, array $options) : \LockmeDep\Psr\Http\Message\UriInterface
    {
        $uri = $request->getUri();
        if (isset($options['force_ip_resolve']) && !\filter_var($uri->getHost(), \FILTER_VALIDATE_IP)) {
            if ('v4' === $options['force_ip_resolve']) {
                ...
                if (\false === $records || !isset($records[0]['ip'])) {
                    throw new \LockmeDep\GuzzleHttp\Exception\ConnectException(\sprintf("Could not resolve IPv4 address for host '%s'", $uri->getHost()), $request);
                }
            ...
            if ('v6' === $options['force_ip_resolve']) {
                ...
                if (\false === $records || !isset($records[0]['ipv6'])) {
                    throw new \LockmeDep\GuzzleHttp\Exception\ConnectException(\sprintf("Could not resolve IPv6 address for host '%s'", $uri->getHost()), $request);
                }
    ...
    private function getDefaultContext(\LockmeDep\Psr\Http\Message\RequestInterface $request) : array
    {
        $headers = '';
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_timeout(\LockmeDep\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if ($value > 0) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_verify(\LockmeDep\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if ($value === \false) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_cert(\LockmeDep\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if (\is_array($value)) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_progress(\LockmeDep\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_debug(\LockmeDep\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if ($value === \false) {
            return;
        }
        ...
        $value = \LockmeDep\GuzzleHttp\Utils::debugResource($value);
