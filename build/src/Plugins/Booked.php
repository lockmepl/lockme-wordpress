...
class Booked implements PluginInterface
{
    ...
    public function GetMessage(array $message): bool
    {
        ...
        $cf_data = ['Source' => sprintf('LockMe (%s)', $data['source']), 'Phone' => $data['phone'], 'People' => $data['people'], 'Price' => $data['price'], 'Comment' => $data['comment'], 'Language' => $data['language']];
        switch ($data['source']) {
            case 'web':
        ...
        switch ($message['action']) {
            case 'add':
                ...
                try {
                    $api = $this->plugin->GetApi();
                    $api->EditReservation($roomid, (string) $lockme_id, $this->plugin->AnonymizeData(['extid' => $row_id]));
                    return \true;
                } catch (Exception) {
                }
                break;
    ...
    private function AppData($res): array
    {
        ...
        if ($res->post_author) {
            ...
            $name = function_exists('quickcal_get_name') ? quickcal_get_name($res->post_author) : booked_get_name($res->post_author);
