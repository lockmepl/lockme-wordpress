...
class Booked implements \LockmeDep\LockmeIntegration\PluginInterface
{
    ...
    public function GetMessage(array $message) : bool
    {
        ...
        $cf_meta_value = '';
        foreach (['Żródło' => 'LockMe', 'Telefon' => $data['phone'], 'Ilość osób' => $data['people'], 'Cena' => $data['price'], 'Status' => $data['status'] ? 'Opłacone' : 'Rezerwacja (max. 20 minut)'] as $label => $value) {
    ...
    public function RegisterSettings() : void
    {
        ...
        add_settings_field('booked_use', 'Włącz integrację', function () {
            echo '<input name="lockme_booked[use]" type="checkbox" value="1"  ' . checked(1, $this->options['use'], \false) . ' />';
