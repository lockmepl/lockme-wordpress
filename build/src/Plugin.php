...
class Plugin
{
    ...
    public function GetApi(): ?Lockme
    {
        ...
        if ($this->options['client_id'] && $this->options['client_secret']) {
            $lm = new Lockme(['provider' => new WrappedProvider(['clientId' => $this->options['client_id'], 'clientSecret' => $this->options['client_secret'], 'redirectUri' => get_admin_url() . 'options-general.php?page=lockme_integration&tab=api_options', 'api_domain' => $this->options['api_domain'] ?: 'https://api.lock.me']), 'tmp_dir' => get_temp_dir(), 'version' => 'v2.4']);
            try {
    ...
    public function admin_register_settings(): void
    {
        ...
        add_settings_field('id_prefix', 'Bookings ID prefix', function () {
